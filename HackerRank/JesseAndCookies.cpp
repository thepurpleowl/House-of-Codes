...
int cookies(int k, vector<int> A)
{
	...
	{
		pq.push(A[i]);  // pushing elements in a priority queue with least element at top.
	}
	int op = 0;
	while (pq.top() < k)  // runs till the smallest element is >= K.
	{
		if (pq.size() < 2)
			return -1;
		op++;
	...
	return op;   // number of operations required.
}
